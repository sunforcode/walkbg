...
interface SegmentRepository : JpaRepository<Segment, String> {
     ...
     * 统计路线的总距离
     */
    @Query("SELECT COALESCE(SUM(s.distance), 0) FROM Segment s WHERE s.routeId = :routeId")
    fun sumDistanceByRouteId(@Param("routeId") routeId: String): Double

    /**
     * 统计路线的总海拔增益
     */
    @Query("SELECT COALESCE(SUM(s.elevationGain), 0) FROM Segment s WHERE s.routeId = :routeId")
    fun sumElevationGainByRouteId(@Param("routeId") routeId: String): Double

    /**
     * 统计路线的总海拔损失
     */
    @Query("SELECT COALESCE(SUM(s.elevationLoss), 0) FROM Segment s WHERE s.routeId = :routeId")
